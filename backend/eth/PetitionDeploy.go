// Code generated - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package blockchain

import (
	"errors"
	"math/big"
	"strings"

	ethereum "github.com/ethereum/go-ethereum"
	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/event"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = errors.New
	_ = big.NewInt
	_ = strings.NewReader
	_ = ethereum.NotFound
	_ = bind.Bind
	_ = common.Big1
	_ = types.BloomLookup
	_ = event.NewSubscription
	_ = abi.ConvertType
)

// PetitContractMetaData contains all meta data concerning the PetitContract contract.
var PetitContractMetaData = &bind.MetaData{
	ABI: "[{\"inputs\":[{\"internalType\":\"string\",\"name\":\"_petition_title\",\"type\":\"string\"},{\"internalType\":\"string\",\"name\":\"_petition_text\",\"type\":\"string\"}],\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"},{\"inputs\":[],\"name\":\"get_petition_text\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"get_petition_title\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"get_signers_count\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"_world_id\",\"type\":\"string\"}],\"name\":\"sign\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"}]",
	Bin: "",
}

// PetitContractABI is the input ABI used to generate the binding from.
// Deprecated: Use PetitContractMetaData.ABI instead.
var PetitContractABI = PetitContractMetaData.ABI

// PetitContractBin is the compiled bytecode used for deploying new contracts.
// Deprecated: Use PetitContractMetaData.Bin instead.
var PetitContractBin = PetitContractMetaData.Bin

// DeployPetitContract deploys a new Ethereum contract, binding an instance of PetitContract to it.
func DeployPetitContract(auth *bind.TransactOpts, backend bind.ContractBackend, _petition_title string, _petition_text string) (common.Address, *types.Transaction, *PetitContract, error) {
	parsed, err := PetitContractMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	if parsed == nil {
		return common.Address{}, nil, nil, errors.New("GetABI returned nil")
	}

	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(PetitContractBin), backend, _petition_title, _petition_text)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &PetitContract{PetitContractCaller: PetitContractCaller{contract: contract}, PetitContractTransactor: PetitContractTransactor{contract: contract}, PetitContractFilterer: PetitContractFilterer{contract: contract}}, nil
}

// PetitContract is an auto generated Go binding around an Ethereum contract.
type PetitContract struct {
	PetitContractCaller     // Read-only binding to the contract
	PetitContractTransactor // Write-only binding to the contract
	PetitContractFilterer   // Log filterer for contract events
}

// PetitContractCaller is an auto generated read-only Go binding around an Ethereum contract.
type PetitContractCaller struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// PetitContractTransactor is an auto generated write-only Go binding around an Ethereum contract.
type PetitContractTransactor struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// PetitContractFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type PetitContractFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// PetitContractSession is an auto generated Go binding around an Ethereum contract,
// with pre-set call and transact options.
type PetitContractSession struct {
	Contract     *PetitContract    // Generic contract binding to set the session for
	CallOpts     bind.CallOpts     // Call options to use throughout this session
	TransactOpts bind.TransactOpts // Transaction auth options to use throughout this session
}

// PetitContractCallerSession is an auto generated read-only Go binding around an Ethereum contract,
// with pre-set call options.
type PetitContractCallerSession struct {
	Contract *PetitContractCaller // Generic contract caller binding to set the session for
	CallOpts bind.CallOpts        // Call options to use throughout this session
}

// PetitContractTransactorSession is an auto generated write-only Go binding around an Ethereum contract,
// with pre-set transact options.
type PetitContractTransactorSession struct {
	Contract     *PetitContractTransactor // Generic contract transactor binding to set the session for
	TransactOpts bind.TransactOpts        // Transaction auth options to use throughout this session
}

// PetitContractRaw is an auto generated low-level Go binding around an Ethereum contract.
type PetitContractRaw struct {
	Contract *PetitContract // Generic contract binding to access the raw methods on
}

// PetitContractCallerRaw is an auto generated low-level read-only Go binding around an Ethereum contract.
type PetitContractCallerRaw struct {
	Contract *PetitContractCaller // Generic read-only contract binding to access the raw methods on
}

// PetitContractTransactorRaw is an auto generated low-level write-only Go binding around an Ethereum contract.
type PetitContractTransactorRaw struct {
	Contract *PetitContractTransactor // Generic write-only contract binding to access the raw methods on
}

// NewPetitContract creates a new instance of PetitContract, bound to a specific deployed contract.
func NewPetitContract(address common.Address, backend bind.ContractBackend) (*PetitContract, error) {
	contract, err := bindPetitContract(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &PetitContract{PetitContractCaller: PetitContractCaller{contract: contract}, PetitContractTransactor: PetitContractTransactor{contract: contract}, PetitContractFilterer: PetitContractFilterer{contract: contract}}, nil
}

// NewPetitContractCaller creates a new read-only instance of PetitContract, bound to a specific deployed contract.
func NewPetitContractCaller(address common.Address, caller bind.ContractCaller) (*PetitContractCaller, error) {
	contract, err := bindPetitContract(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
	return &PetitContractCaller{contract: contract}, nil
}

// NewPetitContractTransactor creates a new write-only instance of PetitContract, bound to a specific deployed contract.
func NewPetitContractTransactor(address common.Address, transactor bind.ContractTransactor) (*PetitContractTransactor, error) {
	contract, err := bindPetitContract(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
	return &PetitContractTransactor{contract: contract}, nil
}

// NewPetitContractFilterer creates a new log filterer instance of PetitContract, bound to a specific deployed contract.
func NewPetitContractFilterer(address common.Address, filterer bind.ContractFilterer) (*PetitContractFilterer, error) {
	contract, err := bindPetitContract(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &PetitContractFilterer{contract: contract}, nil
}

// bindPetitContract binds a generic wrapper to an already deployed contract.
func bindPetitContract(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := PetitContractMetaData.GetAbi()
	if err != nil {
		return nil, err
	}
	return bind.NewBoundContract(address, *parsed, caller, transactor, filterer), nil
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_PetitContract *PetitContractRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _PetitContract.Contract.PetitContractCaller.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_PetitContract *PetitContractRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _PetitContract.Contract.PetitContractTransactor.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_PetitContract *PetitContractRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _PetitContract.Contract.PetitContractTransactor.contract.Transact(opts, method, params...)
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_PetitContract *PetitContractCallerRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _PetitContract.Contract.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_PetitContract *PetitContractTransactorRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _PetitContract.Contract.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_PetitContract *PetitContractTransactorRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _PetitContract.Contract.contract.Transact(opts, method, params...)
}

// GetPetitionText is a free data retrieval call binding the contract method 0x31c07518.
//
// Solidity: function get_petition_text() view returns(string)
func (_PetitContract *PetitContractCaller) GetPetitionText(opts *bind.CallOpts) (string, error) {
	var out []interface{}
	err := _PetitContract.contract.Call(opts, &out, "get_petition_text")

	if err != nil {
		return *new(string), err
	}

	out0 := *abi.ConvertType(out[0], new(string)).(*string)

	return out0, err

}

// GetPetitionText is a free data retrieval call binding the contract method 0x31c07518.
//
// Solidity: function get_petition_text() view returns(string)
func (_PetitContract *PetitContractSession) GetPetitionText() (string, error) {
	return _PetitContract.Contract.GetPetitionText(&_PetitContract.CallOpts)
}

// GetPetitionText is a free data retrieval call binding the contract method 0x31c07518.
//
// Solidity: function get_petition_text() view returns(string)
func (_PetitContract *PetitContractCallerSession) GetPetitionText() (string, error) {
	return _PetitContract.Contract.GetPetitionText(&_PetitContract.CallOpts)
}

// GetPetitionTitle is a free data retrieval call binding the contract method 0x991936f4.
//
// Solidity: function get_petition_title() view returns(string)
func (_PetitContract *PetitContractCaller) GetPetitionTitle(opts *bind.CallOpts) (string, error) {
	var out []interface{}
	err := _PetitContract.contract.Call(opts, &out, "get_petition_title")

	if err != nil {
		return *new(string), err
	}

	out0 := *abi.ConvertType(out[0], new(string)).(*string)

	return out0, err

}

// GetPetitionTitle is a free data retrieval call binding the contract method 0x991936f4.
//
// Solidity: function get_petition_title() view returns(string)
func (_PetitContract *PetitContractSession) GetPetitionTitle() (string, error) {
	return _PetitContract.Contract.GetPetitionTitle(&_PetitContract.CallOpts)
}

// GetPetitionTitle is a free data retrieval call binding the contract method 0x991936f4.
//
// Solidity: function get_petition_title() view returns(string)
func (_PetitContract *PetitContractCallerSession) GetPetitionTitle() (string, error) {
	return _PetitContract.Contract.GetPetitionTitle(&_PetitContract.CallOpts)
}

// GetSignersCount is a free data retrieval call binding the contract method 0x9d737b63.
//
// Solidity: function get_signers_count() view returns(uint256)
func (_PetitContract *PetitContractCaller) GetSignersCount(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _PetitContract.contract.Call(opts, &out, "get_signers_count")

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// GetSignersCount is a free data retrieval call binding the contract method 0x9d737b63.
//
// Solidity: function get_signers_count() view returns(uint256)
func (_PetitContract *PetitContractSession) GetSignersCount() (*big.Int, error) {
	return _PetitContract.Contract.GetSignersCount(&_PetitContract.CallOpts)
}

// GetSignersCount is a free data retrieval call binding the contract method 0x9d737b63.
//
// Solidity: function get_signers_count() view returns(uint256)
func (_PetitContract *PetitContractCallerSession) GetSignersCount() (*big.Int, error) {
	return _PetitContract.Contract.GetSignersCount(&_PetitContract.CallOpts)
}

// Sign is a paid mutator transaction binding the contract method 0x79d6348d.
//
// Solidity: function sign(string _world_id) returns()
func (_PetitContract *PetitContractTransactor) Sign(opts *bind.TransactOpts, _world_id string) (*types.Transaction, error) {
	return _PetitContract.contract.Transact(opts, "sign", _world_id)
}

// Sign is a paid mutator transaction binding the contract method 0x79d6348d.
//
// Solidity: function sign(string _world_id) returns()
func (_PetitContract *PetitContractSession) Sign(_world_id string) (*types.Transaction, error) {
	return _PetitContract.Contract.Sign(&_PetitContract.TransactOpts, _world_id)
}

// Sign is a paid mutator transaction binding the contract method 0x79d6348d.
//
// Solidity: function sign(string _world_id) returns()
func (_PetitContract *PetitContractTransactorSession) Sign(_world_id string) (*types.Transaction, error) {
	return _PetitContract.Contract.Sign(&_PetitContract.TransactOpts, _world_id)
}
